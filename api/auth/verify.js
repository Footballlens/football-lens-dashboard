...
// POST { token }  →  { valid, sessionToken, email }
...
export default async function handler(req, res) {
  ...
  if (!token || !JWT_SECRET) return res.status(400).json({ valid: false, error: "Invalid request" });
  ...
  try {
    ...
    const sessionPayload = Buffer.from(JSON.stringify({ email: decoded.email, exp: sessionExp, type: "session" })).toString("base64url");
